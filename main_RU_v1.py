...
BARCODE = input('Введите трек-номер для отслеживания отправления: ')
...
#ЗАПИСЬ ОСНОВНОЙ ИНФОРМАЦИИ ОБ ОТПРАВИТЕЛЕ, ПОЛУЧАТЕЛЕ И ТИПЕ ОТПРАВКИ
...
main_information = {
    'Трек-номер отправления': BARCODE,
    'Отправитель': sender,
    'Место отправления': send_place,
    'Тип отправления': postal_item,
    'Получатель': recipient,
    'Место назначения': destination_place
}
#ЗАПИСЬ ИНФОРМАЦИИ О ТЕКУЩЕМ СТАТУСЕ ОТПРАВЛЕНИЯ
...
current_status = {
    'Последняя операция': (current_operation, current_attribute),
    'Место операции': current_operation_place,
    'Дата и время последней операции': current_operation_date_time
}
